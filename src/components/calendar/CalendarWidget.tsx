...
import { ChevronLeft, ChevronRight, Calendar, Clock, MapPin, ExternalLink, Plus, Bell, Check, Cake, Activity } from 'lucide-react';
...
const CalendarWidget: React.FC<CalendarWidgetProps> = ({ className = '' }) => {
  ...
  const getCurrentWeekDates = (): Date[] => {
    ...
    // Get all 7 days of the week starting from currentWeekStart (Monday)
    for (let i = 0; i < 7; i++) {
  ...
  const shouldReminderAppearOnDate = (reminder: InternalReminder, targetDate: Date): boolean => {
    ...
    if (!reminder.is_recurring) {
      return reminderStartDateOnly.getTime() === targetDateOnly.getTime();
    }

    // For recurring reminders, check if date is within range
    if (targetDateOnly < reminderStartDateOnly) {
    ...
    const daysDiff = Math.floor((targetDateOnly.getTime() - reminderStartDateOnly.getTime()) / (1000 * 60 * 60 * 24));

    switch (reminder.recurrence_type) {
      case 'daily':
        return true; // Appears every day within range

      case 'weekly':
        return daysDiff % 7 === 0; // Appears every 7 days

      case 'monthly':
        // Appears on the same day of month
        return reminderStartDate.getDate() === targetDate.getDate();

      default:
        return daysDiff === 0; // Unknown type, only show on start date
    }
  };
  ...
  const getItemsForDate = (date: Date): CalendarItem[] => {
    ...
    // Add reminders (including recurring instances)
    reminders.forEach(reminder => {
      if (shouldReminderAppearOnDate(reminder, date)) {
        ...
        items.push({ type: 'reminder', data: reminder, datetime: occurrenceDateTime });
      }
    });
  ...
  const EventItem: React.FC<{ event: CalendarEvent }> = ({ event }) => {
    const calendarNameLower = event.calendarName?.toLowerCase() || '';
      ...
      >
        <div className="flex justify-between items-start">
          <div className="flex items-start flex-1">
            {isBirthday ? (
              <Cake className="h-4 w-4 mr-2 mt-0.5 text-pink-600 dark:text-pink-400 flex-shrink-0" />
            ) : isHealth ? (
              <Activity className="h-4 w-4 mr-2 mt-0.5 text-green-600 dark:text-green-400 flex-shrink-0" />
            ) : (
                      ...
                      {dayItems.map(item => {
                        if (item.type === 'event') {
                          ...
                          return (
                            ...
                            >
                              {isBirthday ? (
                                <Cake className="h-3 w-3 mr-1 text-pink-600 dark:text-pink-400 flex-shrink-0" />
                              ) : isHealth ? (
                                <Activity className="h-3 w-3 mr-1 text-green-600 dark:text-green-400 flex-shrink-0" />
                              ) : (
